...
class LicensingHttpRouter(
    ...
    private val activation: ActivationLicenseHttpEndpoint? = null
) {
    fun handle(request: LicenseHttpRequest): LicenseHttpResponse =
        when (request.path) {
            ...
            ActivationLicenseHttpEndpoint.PATH -> activation?.handle(request)
                ?: LicenseHttpResponse(status = 404, contentType = null, body = byteArrayOf())
