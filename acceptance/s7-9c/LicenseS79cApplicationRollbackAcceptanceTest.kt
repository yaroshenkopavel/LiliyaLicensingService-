...
class LicenseS79cApplicationRollbackAcceptanceTest {
    @Test
    fun version_n_creates_authoritative_state() {
        requirePhase("before")
        val client = client()
        val publicKey = publicKey()

        val issue = assertIs<LicenseClientTransportResult.Signed>(
            client.execute(request(LicenseServiceOperation.ISSUE, "s7-9c-issue-001"))
        )
        ...
        val refresh = assertIs<LicenseClientTransportResult.Signed>(
            client.execute(request(LicenseServiceOperation.REFRESH, "s7-9c-refresh-002"))
        )
        assertEquals(1L, verify(refresh.envelope, publicKey).entitlement.replaySequence?.value)

        writeEvidence(
            "LICENSING_S7_9C_BEFORE_EVIDENCE=" +
                "{\"applicationVersionN\":true,\"issueReplay\":0,\"refreshReplay\":1," +
                "\"frozenClientVerified\":true,\"attemptLimitOne\":true}"
        )
    }

    @Test
    fun rolled_back_version_n_continues_authoritative_state() {
        requirePhase("after")
        val refresh = assertIs<LicenseClientTransportResult.Signed>(
            client().execute(request(LicenseServiceOperation.REFRESH, "s7-9c-refresh-after-rollback-003"))
        )
        assertEquals(2L, verify(refresh.envelope, publicKey()).entitlement.replaySequence?.value)

        writeEvidence(
            "LICENSING_S7_9C_AFTER_EVIDENCE=" +
                "{\"applicationRollback\":true,\"replayAfterRollback\":2," +
                "\"frozenClientVerified\":true,\"stoppedBeforeAuthorityExecution\":true}"
        )
    }

    private fun client(): LicenseHttpTransportClient {
        installAcceptanceTrust(
            Path.of(requiredEnv("LIVE_S79C_TRUSTSTORE_PATH")),
            requiredEnv("LIVE_S79C_TRUSTSTORE_PASSWORD").toCharArray()
        )
        val config = LicenseHttpTransportConfig(
            endpoint = URL(requiredEnv("LIVE_S79C_HTTPS_ENDPOINT")),
        ...
        return LicenseHttpTransportClient(
            config = config,
            engine = BearerLicenseHttpEngine(requiredEnv("LIVE_S79C_BEARER"))
        )
    }

    private fun publicKey(): ByteArray =
        Files.readAllBytes(Path.of(requiredEnv("LIVE_S79C_PUBLIC_KEY_DER_PATH")))
    ...
    private fun request(
        ...
        LicenseServiceTransportRequest(
            ...
            subjectReference = LicenseSubject("s7-9c-rollback-subject"),
    ...
    private fun requirePhase(expected: String) {
        assumeTrue(requiredEnv("LIVE_S79C_PHASE") == expected, "wrong S7.9C phase")
    }

    private fun writeEvidence(value: String) {
        println(value)
        val path = Path.of(requiredEnv("LIVE_S79C_EVIDENCE_PATH"))
    ...
    private fun requiredEnv(name: String): String =
        System.getenv(name)?.takeIf { it.isNotBlank() }
            ?: error("missing S7.9C acceptance environment")
