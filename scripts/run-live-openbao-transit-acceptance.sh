...
OPENBAO_TOKEN="${LIVE_OPENBAO_TOKEN:-liliya-s5-live-only-token}"
...
docker run -d   --name "$CONTAINER_NAME"   -p "$OPENBAO_PORT:8200"   "$OPENBAO_IMAGE"   server -dev   -dev-listen-address=0.0.0.0:8200   -dev-root-token-id="$OPENBAO_TOKEN"   >/dev/null
...
echo "=== ENABLE TRANSIT ==="
HTTP_CODE="$(
  curl -sS -o /tmp/liliya-openbao-mount.out -w '%{http_code}'     -H "X-Vault-Token: $OPENBAO_TOKEN"     -H "Content-Type: application/json"     -X POST     -d '{"type":"transit"}'     "$OPENBAO_ADDR/v1/sys/mounts/transit"
)"
...
curl -fsS   -H "X-Vault-Token: $OPENBAO_TOKEN"   -H "Content-Type: application/json"   -X POST   -d '{"type":"ecdsa-p256","exportable":false,"allow_plaintext_backup":false}'   "$OPENBAO_ADDR/v1/transit/keys/$OPENBAO_KEY"   >/dev/null

echo
echo "=== ROTATE TO VERSION 2 ==="
curl -fsS   -H "X-Vault-Token: $OPENBAO_TOKEN"   -H "Content-Type: application/json"   -X POST   -d '{}'   "$OPENBAO_ADDR/v1/transit/keys/$OPENBAO_KEY/rotate"   >/dev/null

KEY_META="$(
  curl -fsS     -H "X-Vault-Token: $OPENBAO_TOKEN"     "$OPENBAO_ADDR/v1/transit/keys/$OPENBAO_KEY"
)"
...
export LIVE_OPENBAO_TOKEN="$OPENBAO_TOKEN"
